...
void gPlane::setTextureScale(float tileX, float tileY) {
	tilex = tileX;
	tiley = tileY;
	updateTextureCoordinates();
}

void gPlane::setTextureScale(float tile) {
	setTextureScale(tile, tile);
}
...
void gPlane::initializePlane() {
	...
	vertices[0].texcoords = glm::vec2(tilex, 0.0f);
	...
	vertices[1].texcoords = glm::vec2(tilex, tiley);
	...
	vertices[2].texcoords = glm::vec2(0.0f, tiley);
