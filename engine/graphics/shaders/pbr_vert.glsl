...
// The physically based mesh shader, in both dialects.
//
// Vulkan has no default uniform block, so where OpenGL declares loose uniforms and
// a sampler per map, Vulkan declares a push constant block, a scene uniform block
// and its maps as one descriptor set. The lighting below is each backend's own.
