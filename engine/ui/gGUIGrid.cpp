...
void gGUIGrid::set(gBaseApp *root, gBaseGUIObject *topParentGUIObject,
		gBaseGUIObject *parentGUIObject, int parentSlotLineNo,
		int parentSlotColumnNo, int x, int y, int w, int h) {
	gGUIScrollable::set(root, topParentGUIObject, parentGUIObject,
			parentSlotLineNo, parentSlotColumnNo, x, y, w, h);
	setDimensions(w, h);
	textbox.set(root, this, this, 0, 0, gridx + gridboxwhalf + 1,
			gridy + gridboxh - 5, gridboxw - 6, gridboxh);
...
void gGUIGrid::setGrid(int rowNum, int columnNum) {
	...
	if (rowdiff > 0) {
		...
		for (int i = 0; i <= rowdiff; i++) {
			...
			if (hasgridboxesh) {
				gridboxesh.erase(
						std::remove_if(gridboxesh.begin(), gridboxesh.end(),
								[row](const std::array<float, 2> &v) {
									return (int) v[0] == row;
								}), gridboxesh.end());
			}
		}
	...
	if (columndiff > 0) {
		...
		for (int i = 0; i <= columndiff; i++) {
			...
			if (hasgridboxesw) {
				gridboxesw.erase(
						std::remove_if(gridboxesw.begin(), gridboxesw.end(),
								[column](const std::array<float, 2> &v) {
...
void gGUIGrid::setCellFont(int fontNo) {
	...
	if (!ctrlzpressed && !ctrlypressed) {
		...
		for (int i = 0; i < selectedcells.size(); i++) {
			...
			allcells[selectedcells[i]].fontstate =
					gGUIManager::FONTTYPE_REGULAR;
			if (allcells[selectedcells[i]].isbold)
				allcells[selectedcells[i]].fontstate +=
						gGUIManager::FONTTYPE_BOLD;
			if (allcells[selectedcells[i]].isitalic)
				allcells[selectedcells[i]].fontstate +=
						gGUIManager::FONTTYPE_ITALIC;
		}
	} else {
		if (!ctrlypressed) {
			allcells[undocellstack.top()[0]].fontnum = fontNo;
			allcells[undocellstack.top()[0]].fontstate =
					gGUIManager::FONTTYPE_REGULAR;
			...
			allcells[redocellstack.top()[0]].fontstate =
					gGUIManager::FONTTYPE_REGULAR;
	...
	textbox.setTextFont(
			manager->getFont(allcells[selectedbox].fontnum,
					allcells[selectedbox].fontstate/*, allcells[selectedbox].fontsize*/));
}
...
void gGUIGrid::setCellsFont(const std::string &cell1, const std::string &cell2,
		int fontNo) {
...
void gGUIGrid::setCellFontSize(int fontSize) {
	...
	if (!ctrlzpressed && !ctrlypressed) {
		...
		if (!ctrlypressed)
			allcells[undocellstack.top()[0]].fontsize = fontSize;
		else
			allcells[redocellstack.top()[0]].fontsize = fontSize;
	}
}
...
void gGUIGrid::setCellsFontSize(std::deque<Cell*> cells, int fontSize) {
	for (int i = 0; i < cells.size(); i++)
		setCellFontSize(cells[i], fontSize);
}
...
void gGUIGrid::setCellsFontSize(const std::string &cell1,
		const std::string &cell2, int fontSize) {
	Cell *c1 = getCell(cell1);
	Cell *c2 = getCell(cell2);
	...
	if (!ctrlzpressed && !ctrlypressed) {
		...
		for (int i = 0; i < selectedcells.size(); i++) {
			...
			if (fontColor != fontcolor)
				allcells[selectedcells[i]].iscolorchanged = true;
			else
		...
		if (!ctrlypressed) {
			...
			if (fontColor != fontcolor)
				allcells[undocellstack.top()[0]].iscolorchanged = true;
			else
				allcells[undocellstack.top()[0]].iscolorchanged = false;
		} else {
			allcells[redocellstack.top()[0]].cellfontcolor = fontColor;
			if (fontColor != fontcolor)
				allcells[redocellstack.top()[0]].iscolorchanged = true;
			else
				allcells[redocellstack.top()[0]].iscolorchanged = false;
		}
	}
...
void gGUIGrid::setCellFontColor(Cell *cell, gColor *fontColor) {
	...
	if (fontColor != fontcolor)
		cell->iscolorchanged = true;
	else
		cell->iscolorchanged = false;
}
...
void gGUIGrid::setCellsFontColor(std::deque<Cell*> cells, gColor *fontColor) {
	for (int i = 0; i < cells.size(); i++)
		setCellFontColor(cells[i], fontColor);
}
...
void gGUIGrid::setCellsFontColor(const std::string &cell1,
		const std::string &cell2, gColor *fontColor) {
	Cell *c1 = getCell(cell1);
	Cell *c2 = getCell(cell2);
...
void gGUIGrid::setCellLine(int lineNo, bool clicked) {
	...
	if (!ctrlzpressed && !ctrlypressed) {
		...
		for (int i = 0; i < selectedcells.size(); i++) {
			if (clicked && lineNo == allcells[selectedcells[i]].lineno)
				allcells[selectedcells[i]].lineno = TEXTLINE_NONE;
			else
				allcells[selectedcells[i]].lineno = lineNo;
		}
	} else {
		if (!ctrlypressed)
			allcells[undocellstack.top()[0]].lineno = lineNo;
		else
			allcells[redocellstack.top()[0]].lineno = lineNo;
	}
}
void gGUIGrid::setCellLine(Cell *cell, int lineNo, bool clicked) {
	if (clicked && lineNo == cell->lineno)
		cell->lineno = TEXTLINE_NONE;
	else
		cell->lineno = lineNo;
}
...
void gGUIGrid::setCellsLine(std::deque<Cell*> cells, int lineNo, bool clicked) {
	for (int i = 0; i < cells.size(); i++)
		setCellLine(cells[i], lineNo, clicked);
}

void gGUIGrid::setCellsLine(Cell *cell1, Cell *cell2, int lineNo,
		bool clicked) {
	int c1 = cell1->cellcolumnno;
	int c2 = cell2->cellcolumnno;
	int r1 = cell1->cellrowno;
...
void gGUIGrid::setCellsLine(const std::string &cell1, const std::string &cell2,
		int lineNo, bool clicked) {
	Cell *c1 = getCell(cell1);
	Cell *c2 = getCell(cell2);
	setCellsLine(c1, c2, lineNo, clicked);
}

void gGUIGrid::setCellContent(gGUIGrid::Cell *cell,
...
void gGUIGrid::setCellContent(int rowNo, int columnNo,
		const std::string &cellContent) {
...
void gGUIGrid::setCellsContent(Cell *cell1, Cell *cell2,
		std::vector<std::string> contents) {
	int c1 = cell1->cellcolumnno;
	int c2 = cell2->cellcolumnno;
	int r1 = cell1->cellrowno;
...
void gGUIGrid::setCellAlignment(int cellAlignment, bool clicked) {
	...
	if (clicked) {
		...
		for (int i = 0; i < selectedcells.size(); i++) {
			if (clicked && !allcells[selectedcells[i]].iscellaligned)
				allcells[selectedcells[i]].iscellaligned = true;
			else if (clicked && allcells[selectedcells[i]].iscellaligned
					&& allcells[selectedcells[i]].celltype
							== Cell::TYPE_DIGIT) {
				allcells[selectedcells[i]].iscellaligned = false;
				cellAlignment = gBaseGUIObject::TEXTALIGNMENT_RIGHT;
			} else if (clicked
					&& allcells[selectedcells[i]].cellalignment
							== cellAlignment) {
				allcells[selectedcells[i]].iscellaligned = false;
		...
		if (!ctrlzpressed && !ctrlypressed) {
			...
			if (!ctrlypressed) {
				...
				allcells[undocellstack.top()[0]].textmoveamount = 0.5f
						* cellAlignment;
			} else {
				allcells[redocellstack.top()[0]].cellalignment = cellAlignment;
				allcells[redocellstack.top()[0]].textmoveamount = 0.5f
						* cellAlignment;
			}
		}
	}
	textbox.setTextAlignment(cellAlignment, allcells[selectedbox].cellw,
			textbox.getInitX());
}
...
void gGUIGrid::setCellsAlignment(std::deque<Cell*> cells, int cellAlignment) {
	for (int i = 0; i < cells.size(); i++)
		setCellAlignment(cells[i], cellAlignment);
}
...
void gGUIGrid::setCellsAlignment(const std::string &cell1,
		const std::string &cell2, int cellAlignment) {
	Cell *c1 = getCell(cell1);
	Cell *c2 = getCell(cell2);
...
gGUIGrid::Cell* gGUIGrid::getCell(const std::string &cellID) {
	...
	for (int i = 0; i < column.size() - 1; i++) {
		columnindex += int(
				(column[i] % 65 + 1) * (26 * ((column.size() - 1) - i)));
	}
	if (gToInt(row) - 1 < 0 || gToInt(row) - 1 >= rownum || columnindex < 0
...
std::deque<gGUIGrid::Cell*> gGUIGrid::getSelectedCells() {
	std::deque<Cell*> sc;
	for (int i = 0; i < selectedcells.size(); i++)
		sc.push_back(&allcells[selectedcells[i]]);
	return sc;
}
...
std::string gGUIGrid::fixTextFunction(const std::string &text, int index) {
	...
	if (int(tempstr[0]) == 39)
		...
		if (!isdigit(tempstr[isnegative])) {
			...
			if (parentheses1 != std::string::npos
					&& parentheses2 != std::string::npos) {
				std::string functionstr = tempstr.substr(0, parentheses1);
				std::string parenthesesstr = tempstr.substr(parentheses1 + 1,
						parentheses2 - parentheses1 - 1);
				...
				if (functionstr == "SUM") {
					...
					if (doubledat == std::string::npos) {
						if (!isNumeric(parenthesesstr)) {
							...
							std::string row = parenthesesstr.substr(
									column.size());
							for (int i = 0; i < row.size(); i++)
								if (!isdigit(row[i]))
									return errormessage;
							int columnindex = int(column[column.size() - 1])
									% 65;
							for (int i = 0; i < column.size() - 1; i++)
								columnindex += int(
										(column[i] % 65 + 1)
							...
							if (columnindex < 0 || columnindex >= columnnum
									...
									|| gToInt(row) - 1 >= rownum)
								return errormessage;
							int copiedindex = getCellNo(gToInt(row) - 1,
									columnindex);
							if (copiedindex == -1) {
								copiedindex = createCell(gToInt(row) - 1,
										columnindex);
							}
							tempstr = allcells[copiedindex].showncontent;
							functionindexes.clear();
							functionindexes.push_back(
									gToStr(FUNCTIONTYPE_COPY));
						...
						std::string str2 = parenthesesstr.substr(
								str1.size() + 1);
						...
						tempstr = std::to_string(
								makeSum(columnno1, rowno1, columnno2, rowno2));
						addOrChangeFunction(index);
					}
				...
				if (cstr1.find('/') != std::string::npos) {
					...
					if (c1lastindex < cstr1.find('*')
							&& cstr1.find('*') != std::string::npos)
						c1lastindex = cstr1.find('*');
				} else if (cstr1.find('*') != std::string::npos)
					c1lastindex = cstr1.find('*');
				else if (cstr1.find('+') != std::string::npos)
					c1lastindex = cstr1.find('+');
				else if (cstr1.find('-') != std::string::npos)
				...
				if (cstr1.size() != tempstr.size()) {
					...
					if (operation == '+' || operation == '-' || operation == '/'
							|| operation == '*') {
						tempstr = gToStr(
								makeFourOperation(cstr1, cstr2, operation,
										csymbol1, csymbol2));
						functionindexes.clear();
						switch (operation) {
						case '+':
						...
						case '/':
							functionindexes.push_back(
									gToStr(FUNCTIONTYPE_DIVIDE));
							break;
						case '*':
							functionindexes.push_back(
									gToStr(FUNCTIONTYPE_MULTIPLY));
							break;
						}
						functionindexes.push_back(gToStr(index));
					...
					functionindexes.push_back(gToStr(index));
					functionindexes.push_back(
							gToStr(getCellNo(c->cellrowno, c->cellcolumnno)));
					addOrChangeFunction(index);
				}
			}
		} else {
...
std::string gGUIGrid::fixNumeric(const std::string &text) {
	...
	if (tempstr[0] == '-') {
		...
		if (minuscount % 2 == 1)
			tempstr.erase(0, minuscount - 1);
		else
			tempstr.erase(0, minuscount);
	}
	return tempstr;
}
...
std::string gGUIGrid::fixOverflowText(Cell &thisCell, Cell &otherCell) {
	...
	int diff = 0;
	int charindex = 0;
	bool middlealignment = (thisCell.cellalignment
			== gBaseGUIObject::TEXTALIGNMENT_MIDDLE);

	if (otherCell.cellx > thisCell.cellx)
		diff = otherCell.cellx
				- (thisCell.cellx + (middlealignment * thisCell.cellw / 2)
						+ (!middlealignment * (textbox.getInitX() + 2) * 2));
	else
		diff = (thisCell.cellx + thisCell.cellw
				- (middlealignment * thisCell.cellw / 2))
				- (otherCell.cellx + otherCell.cellw
						+ (!middlealignment * (textbox.getInitX() + 2) * 2));
...
int gGUIGrid::getNearestFilledCell(int index) {
	...
	for (int column = 0; column < columnnum; column++) {
		...
		if (cell.cellalignment != gBaseGUIObject::TEXTALIGNMENT_RIGHT
				&& cell.cellx + getFont()->getStringWidth(cell.showncontent)
						> currentcell.cellx
				&& cell.cellcolumnno < currentcell.cellcolumnno) {
			if (nearestindex == -1
					|| allcells[nearestindex].cellcolumnno
							> currentcell.cellcolumnno) {
				nearestindex = currentindex;
			}
		} else if (cell.cellalignment == gBaseGUIObject::TEXTALIGNMENT_RIGHT
				&& cell.cellx
						- (getFont()->getStringWidth(cell.showncontent)
								- cell.cellw)
						< currentcell.cellx + currentcell.cellw
				&& cell.cellcolumnno > currentcell.cellcolumnno) {
			if (nearestindex == -1
					|| allcells[nearestindex].cellcolumnno
							< currentcell.cellcolumnno) {
...
void gGUIGrid::fillCell(int cellNo, const std::string &tempstr) {
	...
	int passindex = 0;
	for (int i = 0; i < functions.size(); i++) {
		if (gToInt(functions[i][FUNCTION_TYPE]) == FUNCTIONTYPE_NONE)
			delindex = i;
		else {
			if (gToInt(functions[i][FUNCTION_TYPE]) == FUNCTIONTYPE_ADD
					|| gToInt(functions[i][FUNCTION_TYPE]) == FUNCTIONTYPE_SUB
					|| gToInt(functions[i][FUNCTION_TYPE])
							== FUNCTIONTYPE_DIVIDE
					|| gToInt(functions[i][FUNCTION_TYPE])
							== FUNCTIONTYPE_MULTIPLY)
				passindex = 2;
			for (int j = FUNCTION_FIRSTINDEX;
					j < functions[i].size() - passindex; j++) {
				if ((passindex != 2 && gToInt(functions[i][j]) == cellNo)
						|| (passindex == 2
								&& getCell(functions[i][j]) == &allcells[cellNo])) {
					operateFunction(i);
					break;
				}
			}
	...
	for (int col = 0; col < columnnum; ++col) {
		...
		if (idx != -1)
			allcells[idx].overflowdirty = true;
	}
	...
	for (int row = r1; row <= r2; row++) {
		for (int column = c1; column <= c2; column++) {
			...
			if (allcells[currentindex].celltype == Cell::TYPE_DIGIT) {
				std::string value = "";
				if (allcells[currentindex].showncontent[0] == '-')
					value += '-';
				for (int i = value.size();
						i < allcells[currentindex].showncontent.size(); i++) {
					if (isdigit(allcells[currentindex].showncontent[i])
							|| allcells[currentindex].showncontent[i] == '.')
						value += allcells[currentindex].showncontent[i];
				}
...
float gGUIGrid::makeFourOperation(const std::string &cell1,
		const std::string &cell2, char operation,
		const std::string &value1symbol, const std::string &value2symbol) {
	Cell *c1 = getCell(cell1);
	Cell *c2 = getCell(cell2);
...
float gGUIGrid::calculateCurrentX(int columnNo) {
	...
	for (int column = 0; column < columnNo; column++)
		currentx += getColumnWidth(column);
	return currentx;
}
...
void gGUIGrid::addUndoStack(int process) {
	...
	if (ctrlypressed) {
		...
		for (int i = 0; i < undocellstack.top().size(); i++) {
			...
			case PROCESS_TEXT:
				undovaluestack.push(
						allcells[undocellstack.top()[i]].cellcontent);
				break;
			case PROCESS_FONT:
				undovaluestack.push(
						std::to_string(
								allcells[undocellstack.top()[i]].fontnum));
				break;
			case PROCESS_FONTSTATE:
				undovaluestack.push(
						std::to_string(
								allcells[undocellstack.top()[i]].fontstate));
				break;
			case PROCESS_FONTSIZE:
				undovaluestack.push(
						std::to_string(
								allcells[undocellstack.top()[i]].fontsize));
				break;
			case PROCESS_ALIGNMENT:
				undovaluestack.push(
						std::to_string(
								allcells[undocellstack.top()[i]].cellalignment));
				break;
			case PROCESS_COLOR:
				undovaluestack.push(
						std::to_string(
								allcells[undocellstack.top()[i]].cellfontcolor.r)
								+ ":"
								+ std::to_string(
										allcells[undocellstack.top()[i]].cellfontcolor.g)
								+ ":"
								+ std::to_string(
										allcells[undocellstack.top()[i]].cellfontcolor.b)
								+ ":"
								+ std::to_string(
										allcells[undocellstack.top()[i]].iscolorchanged));
				break;
			case PROCESS_LINE:
				undovaluestack.push(
						std::to_string(
								allcells[undocellstack.top()[i]].lineno));
				break;
			case PROCESS_ALL:
				undovaluestack.push(
						allcells[undocellstack.top()[i]].cellcontent + ":"
		...
		case PROCESS_FONT:
			...
			std::reverse(undocellstack.top().begin(),
					undocellstack.top().end());
			for (int i = 0; i < selectedcells.size(); i++) {
				undovaluestack.push(
						std::to_string(allcells[selectedcells[i]].fontnum));
			}
			break;
		case PROCESS_FONTSTATE:
			undocellstack.push(selectedcells);
			...
			for (int i = 0; i < selectedcells.size(); i++) {
				undovaluestack.push(
						std::to_string(allcells[selectedcells[i]].fontstate));
			}
			break;
		case PROCESS_FONTSIZE:
			undocellstack.push(selectedcells);
			...
			for (int i = 0; i < selectedcells.size(); i++) {
				undovaluestack.push(
						std::to_string(allcells[selectedcells[i]].fontsize));
			}
			break;
		case PROCESS_ALIGNMENT:
			undocellstack.push(selectedcells);
		...
		case PROCESS_COLOR:
			...
			std::reverse(undocellstack.top().begin(),
					undocellstack.top().end());
			for (int i = 0; i < selectedcells.size(); i++) {
				std::string tmpstr = std::to_string(
						allcells[selectedcells[i]].cellfontcolor.r) + ":"
						+ std::to_string(
								allcells[selectedcells[i]].cellfontcolor.g)
						+ ":"
						+ std::to_string(
								allcells[selectedcells[i]].cellfontcolor.b)
						+ ":"
						+ std::to_string(
								allcells[selectedcells[i]].iscolorchanged);
		...
		case PROCESS_LINE:
			...
			for (int i = 0; i < selectedcells.size(); i++) {
				undovaluestack.push(
						std::to_string(allcells[selectedcells[i]].lineno));
				;
			}
			break;
		case PROCESS_ALL:
			if (!ctrlvpressed) {
				...
				for (int i = 0; i < selectedcells.size(); i++) {
					...
					std::string tmpstr =
							allcells[selectedbox + amount].cellcontent + ":"
									+ std::to_string(
											allcells[selectedbox + amount].fontnum)
									+ ":"
									+ std::to_string(
											allcells[selectedbox + amount].fontstate)
									+ ":"
									+ std::to_string(
											allcells[selectedbox + amount].fontsize)
									+ ":"
									+ std::to_string(
											allcells[selectedbox + amount].cellalignment)
									+ ":"
									+ std::to_string(
											allcells[selectedbox + amount].cellfontcolor.r)
									+ ":"
									+ std::to_string(
											allcells[selectedbox + amount].cellfontcolor.g)
									+ ":"
									+ std::to_string(
											allcells[selectedbox + amount].cellfontcolor.b)
									+ ":"
									+ std::to_string(
											allcells[selectedbox + amount].iscolorchanged)
									+ ":"
									+ std::to_string(
											allcells[selectedbox + amount].lineno);
					undovaluestack.push(tmpstr);
				}
			} else {
				int index = selectedbox;
				int row = (allcells[selectedbox].cellrowno * columnnum
						+ allcells[selectedbox].cellcolumnno) / columnnum;
				int column = (allcells[selectedbox].cellrowno * columnnum
						+ allcells[selectedbox].cellcolumnno) % columnnum;
				int colon =
						copiedcellvalues.at(copiedcellvalues.size() - 1).find(
								':');
				int maxcolumn =
						column
								+ std::stoi(
										copiedcellvalues.at(
												copiedcellvalues.size() - 1).substr(
												0, colon));
				...
				for (int i = 0; i < copiedcellvalues.size(); i++) {
					if (column > maxcolumn) {
						column = (allcells[selectedbox].cellrowno * columnnum
								+ allcells[selectedbox].cellcolumnno)
								% columnnum;
						row++;
					}
...
void gGUIGrid::addRedoStack() {
	...
	for (int i = 0; i < redocellstack.top().size(); i++) {
		...
		case PROCESS_FONTSTATE:
			redovaluestack.push(
					std::to_string(allcells[redocellstack.top()[i]].fontstate));
			break;
		case PROCESS_FONTSIZE:
			redovaluestack.push(
					std::to_string(allcells[redocellstack.top()[i]].fontsize));
			break;
		case PROCESS_ALIGNMENT:
			redovaluestack.push(
					std::to_string(
							allcells[redocellstack.top()[i]].cellalignment));
			break;
		case PROCESS_COLOR:
			redovaluestack.push(
					std::to_string(
							allcells[redocellstack.top()[i]].cellfontcolor.r)
							+ ":"
							+ std::to_string(
									allcells[redocellstack.top()[i]].cellfontcolor.g)
							+ ":"
							+ std::to_string(
									allcells[redocellstack.top()[i]].cellfontcolor.b)
							+ ":"
							+ std::to_string(
									allcells[redocellstack.top()[i]].iscolorchanged));
			break;
		case PROCESS_LINE:
			redovaluestack.push(
					std::to_string(allcells[redocellstack.top()[i]].lineno));
			break;
		case PROCESS_ALL:
			redovaluestack.push(
					allcells[redocellstack.top()[i]].cellcontent + ":"
							+ std::to_string(
									allcells[redocellstack.top()[i]].fontnum)
							+ ":"
							+ std::to_string(
									allcells[redocellstack.top()[i]].fontstate)
							+ ":"
							+ std::to_string(
									allcells[redocellstack.top()[i]].fontsize)
							+ ":"
							+ std::to_string(
									allcells[redocellstack.top()[i]].cellalignment)
							+ ":"
							...
							+ ":"
							+ std::to_string(
									allcells[redocellstack.top()[i]].lineno));
			break;
		}
	}
...
void gGUIGrid::addOrChangeColumnWidth(int columnNo, float w) {
	...
	if (windex == -1)
		gridboxesw.push_back( { (float) columnNo, w });
	else
		gridboxesw[windex][1] = w;
}
...
void gGUIGrid::addOrChangeRowHeight(int rowNo, float h) {
	...
	if (hindex == -1)
		gridboxesh.push_back( { (float) rowNo, h });
	else
		gridboxesh[hindex][1] = h;
}
...
void gGUIGrid::makeDefaultCell() {
	for (int i = 0; i < selectedcells.size(); i++) {
		...
		allcells[selectedcells[i]].cellalignment =
				gBaseGUIObject::TEXTALIGNMENT_LEFT;
		allcells[selectedcells[i]].lineno = TEXTLINE_NONE;
...
void gGUIGrid::operateFunction(int functionNo) {
	...
	case FUNCTIONTYPE_COPY:
		allcells[gToInt(functions[functionNo][FUNCTION_SENDER])].showncontent =
				allcells[gToInt(functions[functionNo][FUNCTION_FIRSTINDEX])].showncontent;
		break;
	case FUNCTIONTYPE_SUM: {
		Cell *c1 = &allcells[gToInt(functions[functionNo][FUNCTION_FIRSTINDEX])];
		Cell *c2 = &allcells[gToInt(
				functions[functionNo][functions[functionNo].size() - 1])];
		allcells[gToInt(functions[functionNo][FUNCTION_SENDER])].showncontent =
				gToStr(
						makeSum(c1->cellcolumnno, c1->cellrowno,
								c2->cellcolumnno, c2->cellrowno));
		break;
	}
	case FUNCTIONTYPE_ADD:
		allcells[gToInt(functions[functionNo][FUNCTION_SENDER])].showncontent =
				gToStr(
						makeFourOperation(
								functions[functionNo][FUNCTION_FIRSTINDEX],
								functions[functionNo][FUNCTION_FIRSTINDEX + 1],
								'+',
								functions[functionNo][FUNCTION_FIRSTINDEX + 2],
								functions[functionNo][FUNCTION_FIRSTINDEX + 3]));
		break;
...
void gGUIGrid::changeSelectedCell(int amount) {
	...
	if (isselected) {
		if (allcells[selectedbox].cellcolumnno
				== firstselectedcell % columnnum) {
			if (amount == 1 && lastselectedcell % columnnum + 1 < columnnum)
				lastselectedcell += amount;
			else if (amount == -1
					&& lastselectedcell % columnnum
							> firstselectedcell % columnnum)
				lastselectedcell += amount;
			else if (amount == -1)
				firstselectedcell += amount;
			else if (amount == columnnum
					&& int(firstselectedcell / columnnum)
							== allcells[selectedbox].cellrowno
					&& lastselectedcell + columnnum < rownum * columnnum)
				lastselectedcell += amount;
			else if (amount == columnnum
					&& firstselectedcell + columnnum <= lastselectedcell
					&& lastselectedcell + columnnum < rownum * columnnum)
				firstselectedcell += amount;
			else if (amount == -columnnum
					&& int(lastselectedcell / columnnum)
							!= allcells[selectedbox].cellrowno
					&& lastselectedcell / columnnum
							> firstselectedcell / columnnum)
				lastselectedcell += amount;
					...
					&& firstselectedcell - columnnum
							>= firstselectedcell % columnnum)
				firstselectedcell += amount;
		} else if (allcells[selectedbox].cellcolumnno
				== lastselectedcell % columnnum) {
			if (amount == 1 && lastselectedcell % columnnum + 1 < columnnum) {
				if (firstselectedcell % columnnum
						< lastselectedcell % columnnum)
					firstselectedcell += amount;
				else
					lastselectedcell += amount;
			} else if (amount == -1 && firstselectedcell % columnnum > 0)
				firstselectedcell += amount;
			else if (amount == columnnum
					&& int(firstselectedcell / columnnum)
							== allcells[selectedbox].cellrowno
		...
		if (amount == 1 && lastselectedcell % columnnum + 1 < columnnum) {
			if (allcells[selectedbox].cellrowno * columnnum
					+ allcells[selectedbox].cellcolumnno == firstselectedcell)
				lastselectedcell += amount;
			else
				firstselectedcell += amount;
		} else if (amount == 1
				&& allcells[selectedbox].cellrowno * columnnum
						+ allcells[selectedbox].cellcolumnno
						!= firstselectedcell)
			firstselectedcell += amount;
		else if (amount == -1
				&& lastselectedcell % columnnum > firstselectedcell
				&& allcells[selectedbox].cellrowno * columnnum
						+ allcells[selectedbox].cellcolumnno
						== firstselectedcell)
			lastselectedcell += amount;
		else if (amount == -1 && firstselectedcell > 0)
			firstselectedcell += amount;
		else if (amount == columnnum
				&& lastselectedcell + columnnum < rownum * columnnum)
			lastselectedcell += amount;
		...
		if (amount == 1 && lastselectedcell % columnnum + 1 < columnnum)
			lastselectedcell += amount;
		else if (amount == -1 && lastselectedcell % columnnum > 0)
			lastselectedcell += amount;
		else if (amount == columnnum
				&& lastselectedcell + columnnum < rownum * columnnum) {
			if (allcells[selectedbox].cellrowno * columnnum
			...
			firstselectedcell += amount;
				...
				&& lastselectedcell / columnnum > firstselectedcell / columnnum
				&& allcells[selectedbox].cellrowno * columnnum
						+ allcells[selectedbox].cellcolumnno
						== firstselectedcell)
			lastselectedcell += amount;
		else if (amount == -columnnum && firstselectedcell > 0)
			firstselectedcell += amount;
	}
	if (firstcellbefore != firstselectedcell) {
		if (calculateCurrentY(firstselectedcell / columnnum)
				< gridy + gridboxh) {
			verticalscroll += calculateCurrentY(firstselectedcell / columnnum)
					- gridboxh;
		} else if (calculateCurrentY(firstselectedcell / columnnum)
				+ getRowHeight(firstselectedcell / columnnum)
				> gridy + boxh - titleheight) {
			verticalscroll += calculateCurrentY(firstselectedcell / columnnum)
					- (gridy + boxh - titleheight)
					+ getRowHeight(firstselectedcell / columnnum);
		} else if (calculateCurrentX(firstselectedcell % columnnum)
				< gridx + gridboxwhalf) {
			horizontalscroll += calculateCurrentX(firstselectedcell % columnnum)
					- gridboxwhalf;
		} else if (calculateCurrentX(firstselectedcell % columnnum)
				+ getColumnWidth(firstselectedcell % columnnum)
				> gridx + boxw) {
			horizontalscroll += calculateCurrentX(firstselectedcell % columnnum)
					- (gridx + boxw)
					+ getColumnWidth(firstselectedcell % columnnum);
		...
		if (calculateCurrentY(lastselectedcell / columnnum)
				< gridy + gridboxh) {
			verticalscroll += calculateCurrentY(lastselectedcell / columnnum)
					- gridboxh;
		} else if (calculateCurrentY(lastselectedcell / columnnum)
				+ getRowHeight(lastselectedcell / columnnum)
				> gridy + boxh - titleheight) {
			verticalscroll += calculateCurrentY(lastselectedcell / columnnum)
					- (gridy + boxh - titleheight)
					+ getRowHeight(lastselectedcell / columnnum);
		} else if (calculateCurrentX(lastselectedcell % columnnum)
				< gridx + gridboxwhalf) {
			horizontalscroll += calculateCurrentX(lastselectedcell % columnnum)
					- gridboxwhalf;
		} else if (calculateCurrentX(lastselectedcell % columnnum)
				+ getColumnWidth(lastselectedcell % columnnum) > gridx + boxw) {
			horizontalscroll += calculateCurrentX(lastselectedcell % columnnum)
					- (gridx + boxw)
					+ getColumnWidth(lastselectedcell % columnnum);
		}
	}
	setSelectedCells();
...
void gGUIGrid::resetSelectedIndexes() {
	selectedcells.clear();
	if (selectedbox > -1) {
		firstselectedcell = allcells[selectedbox].cellrowno * columnnum
				+ allcells[selectedbox].cellcolumnno;
...
void gGUIGrid::copyCells() {
	...
	for (int i = 0; i < selectedcells.size(); i++) {
		...
		std::string tmpstr = std::to_string(cell.cellrowno) + ":"
				+ cell.cellcontent + ":" + std::to_string(cell.fontnum) + ":"
				+ std::to_string(cell.fontstate) + ":"
				+ std::to_string(cell.fontsize) + ":"
				+ std::to_string(cell.cellalignment) + ":"
				+ std::to_string(cell.cellfontcolor.r) + ":"
				+ std::to_string(cell.cellfontcolor.g) + ":"
				+ std::to_string(cell.cellfontcolor.b) + ":"
				+ std::to_string(cell.iscolorchanged) + ":"
...
void gGUIGrid::pasteCells() {
	...
	int maxrow = row
			+ std::stoi(
					copiedcellvalues.at(copiedcellvalues.size() - 1).substr(0,
							colon));
	colon = copiedcellvalues.at(0).find(':');
	...
	for (int i = 0; i < copiedcellvalues.size(); i++) {
		...
		if (copiedcellvalues.size() > i + 1)
			colon = copiedcellvalues.at(i + 1).find(':');
	}
	for (int i = 0; i < tmpval.size(); i++) {
		...
		while (tmpstr != "") {
			...
			case PROCESS_FONTSIZE:
				...
				break;
			case PROCESS_ALIGNMENT:
				allcells[index].cellalignment = std::stoi(
						tmpstr.substr(0, colon));
				allcells[index].textmoveamount = 0.5f
						* std::stoi(tmpstr.substr(0, colon));
				break;
			case PROCESS_LINE:
		...
		row++;
		if (row > maxrow) {
			row = (allcells[selectedbox].cellrowno * columnnum
					+ allcells[selectedbox].cellcolumnno) / columnnum;
			column++;
		}
		index = getCellNo(row, column);
...
void gGUIGrid::makeUndo() {
	...
	while (!undocellstack.top().empty()) {
		...
		if (undoprocessstack.top() == PROCESS_TEXT) {
			fillCell(undocellstack.top().at(0),
					undostack.top().top().substr(0, colon));
			...
			if (std::stoi(undostack.top().top().substr(0, colon))
					- allcells[undocellstack.top()[0]].fontstate == 1
					|| std::stoi(undostack.top().top().substr(0, colon))
							- allcells[undocellstack.top()[0]].fontstate
							== -1) {
				setCellFontBold();
			} else
			...
			setCellAlignment(std::stoi(undostack.top().top().substr(0, colon)),
					false);
			...
			setCellLine(std::stoi(undostack.top().top().substr(0, colon)),
					false);
			...
			if (std::stoi(undostack.top().top().substr(0, colon))
					...
					|| std::stoi(undostack.top().top().substr(0, colon))
							- allcells[undocellstack.top()[0]].fontstate == -1)
				setCellFontBold();
			else if (std::stoi(undostack.top().top().substr(0, colon))
					- allcells[undocellstack.top()[0]].fontstate == 2
					|| std::stoi(undostack.top().top().substr(0, colon))
							- allcells[undocellstack.top()[0]].fontstate == -2)
				setCellFontItalic();
			undostack.top().top().erase(0, colon + 1);
...
void gGUIGrid::makeRedo() {
	...
	while (!redocellstack.top().empty()) {
		...
		if (redoprocessstack.top() == PROCESS_TEXT) {
			fillCell(redocellstack.top().at(0),
					redostack.top().top().substr(0, colon));
			...
			if (std::stoi(redostack.top().top().substr(0, colon))
					- allcells[redocellstack.top()[0]].fontstate == 1
					|| std::stoi(redostack.top().top().substr(0, colon))
							- allcells[redocellstack.top()[0]].fontstate
							== -1) {
				setCellFontBold();
			} else
			...
			setCellAlignment(std::stoi(redostack.top().top().substr(0, colon)),
					false);
			...
			setCellLine(std::stoi(redostack.top().top().substr(0, colon)),
					false);
			...
			if (std::stoi(redostack.top().top().substr(0, colon))
					...
					|| std::stoi(redostack.top().top().substr(0, colon))
							- allcells[redocellstack.top()[0]].fontstate == -1)
				setCellFontBold();
			else if (std::stoi(redostack.top().top().substr(0, colon))
					- allcells[redocellstack.top()[0]].fontstate == 2
					|| std::stoi(redostack.top().top().substr(0, colon))
							- allcells[redocellstack.top()[0]].fontstate == -2)
				setCellFontItalic();
			redostack.top().top().erase(0, colon + 1);
...
void gGUIGrid::createTextBox() {
	...
	//allcells.at(cellindexcounter).cellx + (allcells.at(cellindexcounter).cellw - getFont()->getStringWidth(allcells.at(cellindexcounter).showncontent)) * textbox.getTextMoveAmount() - textbox.getInitX() * textbox.getTextAlignment() - horizontalscroll
	textbox.setEditable(true);
	textbox.set(root, this, this, 0, 0, allcells[selectedbox].cellx + 1,
			allcells[selectedbox].celly - 2, allcells[selectedbox].cellw - 10,
			allcells[selectedbox].cellh - 2);
	textbox.setTextFont(
			manager->getFont(allcells[selectedbox].fontnum,
					allcells[selectedbox].fontstate/*,allcells[selectedbox].fontsize*/));
	textbox.setTextAlignment(allcells[selectedbox].cellalignment,
			allcells[selectedbox].cellw, textbox.getInitX());
	...
	if (allcells[selectedbox].cellcontent != "") {
		...
		if (allcells[selectedbox].cellalignment
				== gBaseGUIObject::TEXTALIGNMENT_LEFT
				|| allcells[selectedbox].cellalignment
						== gBaseGUIObject::TEXTALIGNMENT_RIGHT) {
			textbox.setCursorPosX(
					getFont()->getStringWidth(
							allcells[selectedbox].cellcontent), length);
		} else {
			std::string mid;
			int middle;
			if (length % 2 == 0)
			...
			textbox.setCursorPosX(getFont()->getStringWidth(mid), middle);
		}
...
void gGUIGrid::checkCellType(int cellIndex) {
	if (allcells[cellIndex].showncontent == "")
		return;
	bool digit;
	bool isdate;
	if (int(allcells[cellIndex].cellcontent[0]) != 39) {
		...
		if (allcells[cellIndex].showncontent[0] == '-')
			isnegative = true;
		if (isdigit(allcells[cellIndex].showncontent[isnegative])) {
			for (int i = 1 + isnegative;
					i < allcells[cellIndex].showncontent.length(); i++) {
				if (allcells[cellIndex].showncontent[i] == '.') {
					...
					isfractional = true;
					for (int j = i + 1;
							j < allcells[cellIndex].showncontent.length();
							j++) {
		...
		if (digit) {
			if (isfractional) {
				...
				if (dotindex != -1)
					allcells[cellIndex].showncontent.erase(dotindex,
							allcells[cellIndex].showncontent.length()
									- dotindex);
			}
			if (spaceindexes.empty()) {
				std::string tmpstr;
				if (isfractional
						&& allcells[cellIndex].showncontent.find('.') != -1)
					tmpstr = allcells[cellIndex].showncontent.substr(0,
							allcells[cellIndex].showncontent.find('.'));
				else
					tmpstr = allcells[cellIndex].showncontent;
				int next = 3;
				std::deque<int> indexes;
				...
				if (isfractional
						&& allcells[cellIndex].showncontent.find('.') != -1)
					allcells[cellIndex].showncontent = tmpstr
							+ allcells[cellIndex].showncontent.substr(
									allcells[cellIndex].showncontent.find('.'));
				else
					allcells[cellIndex].showncontent = tmpstr;
			} else {
				while (!spaceindexes.empty()) {
					if (allcells[cellIndex].showncontent[spaceindexes.top()]
							!= ',')
						allcells[cellIndex].showncontent[spaceindexes.top()] =
								',';
					spaceindexes.pop();
				}
			}
			...
			if (isdate) {
				...
				if (day < 1 || day > 31 || month < 1 || month > 12 || year < 0)
					...
					if (allcells[cellIndex].showncontent[2] != '.')
						allcells[cellIndex].showncontent[2] = '.';
					if (allcells[cellIndex].showncontent[5] != '.')
						allcells[cellIndex].showncontent[5] = '.';
				}
			}
			...
			if (isdate) {
				...
				std::string month = allcells[cellIndex].showncontent.substr(9,
						3);
				...
				if (day < 1 || day > 31 || mon < 1 || mon > 12 || year < 0)
					...
					std::transform(allcells[cellIndex].showncontent.begin(),
							allcells[cellIndex].showncontent.end(),
							allcells[cellIndex].showncontent.begin(),
							::toupper);
			}
		} else
			isdate = false;
	...
	if (!digit && !isdate) {
		if (allcells[cellIndex].celltype == Cell::TYPE_DIGIT
				&& !allcells[cellIndex].iscellaligned)
			setCellAlignment(&allcells[cellIndex],
					gBaseGUIObject::TEXTALIGNMENT_LEFT);
		allcells[cellIndex].celltype = Cell::TYPE_STRING;
	} else {
		allcells[cellIndex].celltype = Cell::TYPE_DIGIT;
		if (!allcells[cellIndex].iscellaligned)
			setCellAlignment(&allcells[cellIndex],
					gBaseGUIObject::TEXTALIGNMENT_RIGHT);
	}
}
...
//	gLogi("Scrollable") << "drawContent fx:" << horizontalscroll << ", fy:" << verticalscroll;
	...
	gDrawLine(gridx + gridboxwhalf + 1, gridy, gridx + gridboxwhalf + 1,
			gridh + gridboxh - verticalscroll);
	gDrawLine(gridx, gridy + gridboxh, gridw + gridboxwhalf - horizontalscroll,
			gridy + gridboxh);
...
void gGUIGrid::drawCellBackground() {
	...
	gDrawRectangle(gridx + gridboxwhalf, gridy + gridboxh,
			gridw - horizontalscroll, gridh - verticalscroll, true);
}

void gGUIGrid::drawRowHeader() {
	// draw header
	renderer->setColor(buttoncolor);
	gDrawRectangle(gridx, gridy + gridboxh, gridboxwhalf,
			gridh - verticalscroll, true);
	...
	for (int i = 0; i < rownum; i++) {
		...
		getFont()->drawText(rowtitlestring,
				gridx + (gridboxw / 4)
						- (getFont()->getStringWidth(rowtitlestring) / 2),
				currenty - (prevh / 2)
						+ (getFont()->getStringHeight(rowtitlestring) / 2));
		renderer->setColor(pressedbuttoncolor);
...
void gGUIGrid::drawRowLines() {
	...
	for (int i = 0; i < rownum; i++) {
		...
		renderer->setColor(pressedbuttoncolor);
		gDrawLine(gridx - horizontalscroll + gridboxwhalf, currenty,
				gridx + gridw + gridboxwhalf - horizontalscroll, currenty);
	}
}

void gGUIGrid::drawColumnHeader() {
	renderer->setColor(buttoncolor);
	gDrawRectangle(gridx + gridboxwhalf, gridy, gridw - horizontalscroll,
			gridboxh, true);
	...
	for (int i = 0; i < columnnum; i++) {
		...
		renderer->setColor(fontcolor);
		getFont()->drawText(columntitlestring,
				currentx - (prevw / 2)
						- (getFont()->getStringWidth(columntitlestring) / 2),
				gridy + (gridboxh / 2)
						+ (getFont()->getStringHeight(columntitlestring) / 2));
		renderer->setColor(pressedbuttoncolor);
		gDrawLine(currentx, gridy, currentx, gridy + gridboxh);
	}
}
...
void gGUIGrid::drawColumnLines() {
	...
	for (int i = 0; i < columnnum; i++) {
		...
		renderer->setColor(pressedbuttoncolor);
		gDrawLine(currentx, gridy - verticalscroll + gridboxh, currentx,
				gridy + gridboxh + gridh - verticalscroll);
	}
}
...
void gGUIGrid::drawCellContents() {
	...
	for (int i = 0; i < allcells.size(); i++) {
		...
		gFont &cellfont = *manager->getFont(currentcell.fontnum,
				currentcell.fontstate/*, currentcell.fontsize*/);
		int shownwidth = cellfont.getStringWidth(currentcell.showncontent);
		if (currentcell.cellx + shownwidth * currentcell.textmoveamount
				- textbox.getInitX() * currentcell.cellalignment
				< gridx + horizontalscroll) {
			continue;
		}
		...
		if (currentcell.overflowdirty) {
			...
			if (currentcell.cellw < shownwidth) {
				...
				currentcell.overflowcontent =
						(nearestindex != -1) ?
								fixOverflowText(currentcell,
										allcells[nearestindex]) :
								"";
			} else {
				currentcell.overflowcontent = "";
			}
		...
		if (currentcell.overflowcontent.empty()) {
					...
					currentstringheight = cellfont.getStringHeight(
							currentcell.showncontent);

					cellfont.drawText(currentcell.showncontent,
							currentcell.cellx
									+ (currentcell.cellw - currentstringwidth)
											* currentcell.textmoveamount
									- textbox.getInitX() * currentcell.cellalignment
									- horizontalscroll,
							currentcell.celly + (gridboxh / 2)
									+ (currentstringheight / 2) - verticalscroll);
				} else {
					currentstringwidth = cellfont.getStringWidth(
							currentcell.overflowcontent);
					currentstringheight = cellfont.getStringHeight(
							currentcell.overflowcontent);
		...
		case TEXTLINE_UNDER:
			gDrawLine(
					currentcell.cellx
							...
							- textbox.getInitX() * currentcell.cellalignment
							- horizontalscroll,
					currentcell.celly + (gridboxh / 2) + currentstringheight
							- verticalscroll,
					currentcell.cellx + currentstringwidth
							+ ((currentcell.cellw - currentstringwidth)
									* currentcell.textmoveamount)
							- horizontalscroll,
					currentcell.celly + (gridboxh / 2) + currentstringheight
			...
			gDrawLine(
					...
					currentcell.cellx + currentstringwidth
							+ ((currentcell.cellw - currentstringwidth)
									* currentcell.textmoveamount)
							- horizontalscroll,
					currentcell.celly + currentcell.cellh / 2
							+ currentstringheight - verticalscroll);
			gDrawLine(
					currentcell.cellx
							+ (currentcell.cellw - currentstringwidth)
									* currentcell.textmoveamount
		...
		case TEXTLINE_STRIKE:
			gDrawLine(
					currentcell.cellx
							...
							- horizontalscroll,
					currentcell.celly + currentcell.cellh / 2
							+ textbox.getInitX() - verticalscroll,
					currentcell.cellx + currentstringwidth
							+ ((currentcell.cellw - currentstringwidth)
									* currentcell.textmoveamount)
							- horizontalscroll,
...
void gGUIGrid::drawSelectedArea() {
	if (columnnum <= 0 || firstselectedcell < 0 || lastselectedcell < 0)
		return;
	int sx = calculateCurrentX(firstselectedcell % columnnum);
	int sy = calculateCurrentY(int(firstselectedcell / columnnum));
	int sw = calculateCurrentX(lastselectedcell % columnnum) - sx
			+ getColumnWidth(lastselectedcell % columnnum);
	int sh = calculateCurrentY(int(lastselectedcell / columnnum)) - sy
			+ getRowHeight(int(lastselectedcell / columnnum));
	if (appmanager->getGUIManager()->getTheme() == gGUIManager::GUITHEME_DARK)
		renderer->setColor(selectedareadarkcolor);
	else
		renderer->setColor(selectedareacolor);
	gDrawRectangle(sx, sy, sw, sh, true);
	if (selectedbox >= 0 && selectedbox < (int) allcells.size()) {
		renderer->setColor(*textbackgroundcolor);
		gDrawRectangle(allcells[selectedbox].cellx - horizontalscroll,
				allcells[selectedbox].celly - verticalscroll,
				allcells[selectedbox].cellw, allcells[selectedbox].cellh, true);
	}
...
void gGUIGrid::mousePressed(int x, int y, int button) {
	...
	if (cursor == gGUIForm::CURSOR_VRESIZE
			|| cursor == gGUIForm::CURSOR_HRESIZE) {
		firstcursorposx = x;
		firstcursorposy = y;
		...
		if (clicktime - previousclicktime <= clicktimediff) {
			if (cursor == gGUIForm::CURSOR_VRESIZE) {
				int row = 0;
				while (calculateCurrentY(row) + getRowHeight(row)
						< pressedy - mousetolerance - verticalscroll) {
					row++;
				}
				...
				int column = 0;
				while (calculateCurrentX(column) + getColumnWidth(column)
						< pressedx - mousetolerance - horizontalscroll)
					column++;
				...
				Cell &cell = allcells[index];
				gFont &cellfont = *manager->getFont(cell.fontnum,
						cell.fontstate/*, allcells[index].fontsize*/);
				for (int i = 1; i < rownum; ++i) {
					...
					gFont &loopcellfont =
							*manager->getFont(loopcell.fontnum,
									loopcell.fontstate/*, allcells[loopcellindex].fontsize*/);
					if (loopcellfont.getStringWidth(loopcell.showncontent)
							> cellfont.getStringWidth(cell.showncontent)) {
						index = i;
					}
				}
				cell = allcells[index];
				float neww = cellfont.getStringWidth(cell.showncontent)
						+ textbox.getInitX() + 1;
				if (neww < getFont()->getSize() * 1.8f)
					neww = getFont()->getSize() * 1.8f;
	...
	} else if (pressedx >= gridx + horizontalscroll
			&& pressedx < gridx + gridboxwhalf + horizontalscroll
			&& pressedy >= gridy + verticalscroll
			&& pressedy < gridy + gridboxh + verticalscroll) {
		isselected = true;
		isrowselected = true;
		...
		setSelectedCells();
			...
			&& pressedx <= gridx + gridboxwhalf + gridw
			&& pressedy >= gridy + verticalscroll
			&& pressedy <= gridy + gridboxh + gridh) {
		if (pressedx >= gridx + gridboxwhalf + horizontalscroll
				&& pressedx <= gridx + gridboxwhalf + gridw
				&& pressedy >= gridy + gridboxh + verticalscroll
				&& pressedy <= gridy + gridboxh + gridh) {
			isselected = true;
			...
			int column = 0;
			while (calculateCurrentY(row) + getRowHeight(row)
					< pressedy - verticalscroll)
				row++;
			while (calculateCurrentX(column) + getColumnWidth(column)
					< pressedx - horizontalscroll)
				column++;
			int index = getCellNo(row, column);
			if (index == -1) {
				index = createCell(row, column);
			...
			root->getCurrentCanvas()->onGuiEvent(id, G_GUIEVENT_CELLSELECTED,
					gToStr(row), gToStr(column));
		} else if (pressedx >= gridx + horizontalscroll
				&& pressedx < gridx + gridboxwhalf + horizontalscroll
				&& pressedy >= gridy + gridboxh + verticalscroll
				&& pressedy <= gridy + gridboxh + gridh) {
			...
			setSelectedCells();
		} else if (pressedx >= gridx + gridboxwhalf + horizontalscroll
				&& pressedx < gridx + gridboxwhalf + gridw
				&& pressedy >= gridy + verticalscroll
				&& pressedy <= gridy + gridboxh + verticalscroll) {
			int column = 0;
...
void gGUIGrid::mouseReleased(int x, int y, int button) {
	if (istextboxactive)
		textbox.mouseReleased((x - left),
				(y - top - verticalscroll - titleheight), button);
...
//	gLogi("Grid") << x << " " << y;
	if (istextboxactive)
		textbox.mouseDragged((x - left), (y - top - verticalscroll), button);
	...
	if (cursor == gGUIForm::CURSOR_VRESIZE) {
		...
		if (!iscolumnselected)
			while (calculateCurrentY(row) + getRowHeight(row)
				...
				row++;
		if (!isrowselected)
			while (calculateCurrentX(column) + getColumnWidth(currentcolumn)
					< x - left)
				column++;
		int draggedcell = row * columnnum
				+ ((rownum - 1) * columnnum * iscolumnselected) + column
				+ columnnum * isrowselected;
		int amount;
		if (lastdraggedcell == -1)
			lastdraggedcell = lastselectedcell;
...
void gGUIGrid::keyPressed(int key) {
	if (istextboxactive)
	...
	} else if (key == G_KEY_B && ctrlpressed)
		setCellFontBold();
	else if (key == G_KEY_I && ctrlpressed)
		setCellFontItalic();
	else if (shiftpressed) {
		if (key == G_KEY_DOWN)
			changeSelectedCell(columnnum);
			...
			&& (key == G_KEY_LEFT_CONTROL || key == G_KEY_RIGHT_CONTROL))
		ctrlpressed = true;
	else if ((isselected || isrowselected || iscolumnselected)
			&& (key == G_KEY_LEFT_SHIFT || key == G_KEY_RIGHT_SHIFT)) {
		shiftpressed = true;
		if (isselected && firstselectedcell == -1) {
			...
			firstselectedcell = allcells[selectedbox].cellrowno * columnnum
					+ allcells[selectedbox].cellcolumnno;
			lastselectedcell = allcells[selectedbox].cellrowno * columnnum
					+ columnnum - 1;
		} else if (iscolumnselected && firstselectedcell == -1) {
			firstselectedcell = allcells[selectedbox].cellrowno * columnnum
					+ allcells[selectedbox].cellcolumnno;
			lastselectedcell = (rownum - 1) * columnnum
					+ allcells[selectedbox].cellcolumnno;
		}
	} else if ((isselected || isrowselected || iscolumnselected) && !ctrlpressed
			&& key != G_KEY_ENTER && key != G_KEY_UP && key != G_KEY_DOWN
			&& key != G_KEY_RIGHT && key != G_KEY_LEFT && key != G_KEY_ESC
			&& key != G_KEY_F2) {
		editCell(allcells[selectedbox], true);
		if (istextboxactive) {
			textbox.mousePressed(
					allcells[selectedbox].cellx + textbox.getInitX(),
					allcells[selectedbox].celly + textbox.getInitX(), 0);
			textbox.keyPressed(key);
		}
	}
}
...
void gGUIGrid::keyReleased(int key) {
	...
	if (key == G_KEY_ENTER && isdoubleclicked
			&& (strflag != allcells[selectedbox].cellcontent
					|| (strflag == "" && textbox.getText() != "")))
		addUndoStack(PROCESS_TEXT);
	if ((key == G_KEY_ENTER && firstselectedcell != lastselectedcell)) {
		changeCell(selectedbox);
		disableTextbox();
		bool godown = (allcells[selectedbox].cellrowno + 1
				<= int(lastselectedcell / columnnum));
		int newrow;
		int newcolumn;
		if (!godown) {
			...
			if (allcells[selectedbox].cellcolumnno
				...
				newcolumn = firstselectedcell % columnnum;
		} else {
		...
		if (!shiftpressed && allcells[selectedbox].cellrowno - 1 >= 0) {
			isrowselected = false;
			isselected = true;
			int index = getCellNo(allcells[selectedbox].cellrowno - 1,
					allcells[selectedbox].cellcolumnno);
			if (index == -1) {
				index = createCell(allcells[selectedbox].cellrowno - 1,
						allcells[selectedbox].cellcolumnno);
			}
			selectedbox = index;
			resetSelectedIndexes();
		...
		if (!shiftpressed
				&& allcells[selectedbox].cellcolumnno + 1 < columnnum) {
			iscolumnselected = false;
			isselected = true;
			int index = getCellNo(allcells[selectedbox].cellrowno,
					allcells[selectedbox].cellcolumnno + 1);
			if (index == -1) {
				index = createCell(allcells[selectedbox].cellrowno,
						allcells[selectedbox].cellcolumnno + 1);
			}
			selectedbox = index;
			resetSelectedIndexes();
		...
		editCell(allcells[selectedbox]);
	} else if (key == G_KEY_LEFT_CONTROL || key == G_KEY_RIGHT_CONTROL)
		ctrlpressed = false;
	else if (key == G_KEY_LEFT_SHIFT || key == G_KEY_RIGHT_SHIFT)
		shiftpressed = false;
}
...
int gGUIGrid::getCursor(int x, int y) {
	...
	currentrow = 0;
	currentcolumn = 0;
	if (currentx >= gridx + horizontalscroll
			&& currentx < gridx + gridboxwhalf + horizontalscroll
			&& currenty
					>= gridy + gridboxh + getRowHeight(currentrow)
							- mousetolerance - verticalscroll
			&& currenty
					<= gridy + gridh + mousetolerance + getRowHeight(rownum)) {
		int currenth = gridy + gridboxh + getRowHeight(currentrow)
				- mousetolerance;
		while (currentrow + 1 < rownum
				&& currenth + getRowHeight(currentrow + 1) <= currenty) {
			currenth += getRowHeight(currentrow + 1);
			currentrow++;
		}
		if (currenth <= currenty && currenth + mousetolerance * 2 >= currenty)
			cursor = gGUIForm::CURSOR_VRESIZE;
	} else if (currentx
			>= gridx + gridboxwhalf + getColumnWidth(currentcolumn)
					- mousetolerance - horizontalscroll
			&& currentx
					< gridx + gridw + mousetolerance + getColumnWidth(columnnum)
			&& currenty >= gridy + verticalscroll
			&& currenty <= gridy + gridboxh + verticalscroll) {
		int currentw = gridx + gridboxwhalf + getColumnWidth(currentcolumn)
				- mousetolerance;
		while (currentcolumn + 1 < columnnum
				&& currentw + getColumnWidth(currentcolumn + 1) <= currentx) {
			currentw += getColumnWidth(currentcolumn + 1);
			currentcolumn++;
		}
		if (currentw <= currentx && currentw + mousetolerance * 2 >= currentx)
			cursor = gGUIForm::CURSOR_HRESIZE;
	}
	return cursor;
}
