...
// everything about the procedural sunset skybox lives in this one function
void generatesunsetskybox(gSkybox& sky, const glm::vec3& sundir, int facesize) {
	auto facedirection = [](int face, float s, float t) {
		switch(face) {
	...
	auto colorat = [&sundir](const glm::vec3& dir) {
	...
	for(int face = 0; face < 6; face++) {
		...
		for(int y = 0; y < facesize; y++) {
			...
			for(int x = 0; x < facesize; x++) {
				...
				glm::vec3 dir = facedirection(face, s, t);
				glm::vec3 color = colorat(dir);
...
// y positions below keep each model's own lowest vertex resting on the floor, since the kit's models are not uniformly centered
void gCanvas::setupshowcase() {
	doorframe.loadModel("ModularSciFiKit/OBJ/Platforms/Door_Frame_Square.obj");
	doorframe.setPosition(0.0f, 0.013425f, -7.0f);

	columnastraleft.loadModel("ModularSciFiKit/OBJ/Columns/Column_Astra.obj");
	columnastraleft.setPosition(-3.6f, 0.006465f, -7.0f);

	columnastraright.loadModel("ModularSciFiKit/OBJ/Columns/Column_Astra.obj");
	columnastraright.setPosition(3.6f, 0.006465f, -7.0f);

	columnhollow.loadModel("ModularSciFiKit/OBJ/Columns/Column_Hollow.obj");
	columnhollow.setPosition(-6.0f, 0.0f, -3.5f);

	columnround.loadModel("ModularSciFiKit/OBJ/Columns/Column_Round.obj");
	columnround.setPosition(6.0f, 0.0f, -3.5f);

	crate3.loadModel("ModularSciFiKit/OBJ/Props/Prop_Crate3.obj");
	crate3.setPosition(-2.0f, 0.5f, 3.0f);
	crate3.rotateDeg(15.0f, 0.0f, 1.0f, 0.0f);

	crate4.loadModel("ModularSciFiKit/OBJ/Props/Prop_Crate4.obj");
	crate4.setPosition(-1.0f, 0.560435f, 3.6f);
	crate4.rotateDeg(-25.0f, 0.0f, 1.0f, 0.0f);

	chest.loadModel("ModularSciFiKit/OBJ/Props/Prop_Chest.obj");
	chest.setPosition(2.6f, -0.003236f, 3.8f);
	chest.rotateDeg(20.0f, 0.0f, 1.0f, 0.0f);
}
...
void gCanvas::drawscene() {
	...
	showcasesphere.draw();
	doorframe.draw();
	columnastraleft.draw();
	columnastraright.draw();
	columnhollow.draw();
	columnround.draw();
	crate3.draw();
	crate4.draw();
